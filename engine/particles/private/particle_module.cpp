...
bool ParticleModule::SetEmitterPresetImage(EmitterPreset& preset)
{
    ...
    float biggestSize = glm::max(resources->GetImageResourceManager().get(image)->width, resources->GetImageResourceManager().get(image)->height);
    preset.size = glm::vec3(
        resources->GetImageResourceManager().get(image)->width / preset.spriteDimensions.x / biggestSize,
        resources->GetImageResourceManager().get(image)->height / preset.spriteDimensions.y / biggestSize, preset.size.z);
