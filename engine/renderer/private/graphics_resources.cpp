...
{
    _samplerResourceManager = std::make_shared<bb::SamplerManager>(*_vulkanContext);
    _bufferResourceManager = std::make_shared<bb::BufferManager>(*_vulkanContext);

    _imageResourceManager = std::make_shared<ImageResourceManager>(_vulkanContext, _samplerResourceManager->GetDefaultSampler());
