...
static vk::ImageType ImageTypeConversion(ImageType type)
{
    switch (type)
    {
    case ImageType::e2D:
...
static vk::ImageViewType ImageViewTypeConversion(ImageType type)
{
    switch (type)
    {
    case ImageType::eShadowMap:
    ...
    SingleTimeCommands* commands)
{
    ...
    out.width = creation.width;
    out.height = creation.height;
    out.depth = creation.depth;
    out.layers = creation.layers;
    out.flags = creation.flags;
    out.type = creation.type;
    out.format = creation.format;
    out.mips = std::min(creation.mips, static_cast<uint8_t>(floor(log2(std::max(out.width, out.height))) + 1));
    out.name = creation.name;
    out.isHDR = creation.isHDR;
    out.sampler = sampler;
...

    {
        ...
        util::vmaCreateImage(_context->MemoryAllocator(), reinterpret_cast<VkImageCreateInfo*>(&imageCreateInfo), &allocCreateInfo, reinterpret_cast<VkImage*>(&out.handle), &out.allocation, nullptr);
        std::string allocName = creation.name + " handle allocation";
        vmaSetAllocationName(_context->MemoryAllocator(), out.allocation, allocName.c_str());
    }
    ...
    if (creation.initialData.data())
    {
        ...
        vk::DeviceSize imageSize = out.width * out.height * out.depth * 4;

        if (out.format == vk::Format::eR8Unorm)
        {
            imageSize = out.width * out.height * out.depth;
        }
        if (out.isHDR)
        {
        ...
        {
            ZoneScopedN("Command upload dispatch");
            vk::CommandBuffer commandBuffer = util::BeginSingleTimeCommands(out._context);
...

            {
                ...
                util::EndSingleTimeCommands(out._context, commandBuffer);
            }
